...
utils.proxySpecifiedRequest = function(req, res, next) {
    var isNexted = false;

    proxies.forEach(function(proxy) {
        if (proxy.config.methods != null) {
            if (!_.isArray(proxy.config.methods)) {
                proxy.config.methods = [proxy.config.methods];
            }
            _.forEach(proxy.config.methods, function(value) {
                if (req.method.toUpperCase() === value.toUpperCase()) {
                    isNexted = true;
